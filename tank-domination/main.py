...
game_characteristics = {
    ...
    "player" :{
        "position": {
            "x" : 0.20 * monitor_display[0]
        },
        "hp":1
    },
    "cpu" :{
        "position": {
            "x" : 0.80 * monitor_display[0] - game_tank_sprite.get_width()
        },
        "hp":1
    }
}
...
while game_running_flag:
    ...
    if 0 <= game_characteristics["player"]["position"]["x"] + position_delta and game_characteristics["player"]["position"]["x"] + position_delta + game_tank_sprite.get_width() <= game_characteristics
