...
for current_answer_character in answer:
    if re.search("^[A-Z]$", current_answer_character):
        answered_guessed.append(False)
    else :
        answered_guessed.append(True)
...
while current_incorrect_guesses < num_of_incorrect_guesses and False in answered_guessed:
#game summary
...
#display puzzle board
    for current_answer_index in range(len(answer)):
        if answered_guessed[current_answer_index]:
            print(answer[current_answer_index], end="")
        else:
            print("_", end="")
    print()

    #let user guessa letter
        ...
        if letter in answer:
            for current_answer_index in range (len(answer)):
                if letter == answer[current_answer_index]:
                    answered_guessed[current_answer_index]=True
        else:
